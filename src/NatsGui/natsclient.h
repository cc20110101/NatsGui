...
using MessageCallback = std::function < void(QString && message, QString
                                             && inbox, QString && subject) >;
...
class Client : public QObject {
  ...
  public:
    ...
    uint64_t subscribe(const QString &subject, const QString &queue,
    ...
    uint64_t request(const QString subject, const QString message,
                     MessageCallback callback);
    ...
    //! determine whether connect is isValid
    bool isOpen();
  ...
  public slots:
    ...
    void connect(const QString &host = "127.0.0.1", quint16 port = 4222,
                 ConnectCallback callback = nullptr);
    void connect(const QString &host, quint16 port, const Options &options,
                 ConnectCallback callback = nullptr);
...
inline Client::Client(QObject *parent) : QObject(parent) {
    ...
    if(_debug_mode)
        DEBUG("debug mode");
    // 报错the proxy type is invalid for this operation,去掉代理
    m_socket.setProxy(QNetworkProxy::NoProxy);
}

inline void Client::connect(const QString &host, quint16 port,
                            ConnectCallback callback) {
    connect(host, port, m_options, callback);
}

inline void Client::connect(const QString &host, quint16 port,
                            const Options &options, ConnectCallback callback) {
    ...
    (&QSslSocket::sslErrors), [this](const QList<QSslError> &errors) {
        DEBUG(errors);
    ...
    *signal = QObject::connect(&m_socket, &QSslSocket::readyRead, [this, signal,
          options, callback] {
...
inline void Client::publish(const QString &subject, const QString &message,
                            const QString &inbox) {
    QString body = QStringLiteral("PUB ") % subject % " " % inbox %
                   (inbox.isEmpty() ? "" : " ") % QString::number(message.length()) % CLRF %
                   message % CLRF;
...
inline uint64_t Client::subscribe(const QString &subject,
                                  MessageCallback callback) {
    return subscribe(subject, "", callback);
}

inline uint64_t Client::subscribe(const QString &subject, const QString &queue,
                                  MessageCallback callback) {
    m_callbacks[++m_ssid] = callback;

    QString message = QStringLiteral("SUB ") % subject % " " % queue %
                      (queue.isEmpty() ? "" : " ") % QString::number(m_ssid) % CLRF;
...
inline Subscription *Client::subscribe(const QString &subject) {
    ...
    subscription->ssid = subscribe(subject,
                                   "", [subscription](const QString & message, const QString & subject,
    const QString & inbox) {
...
inline void Client::unsubscribe(uint64_t ssid, int max_messages) {
    QString message = QStringLiteral("UNSUB ") % QString::number(ssid) %
                      (max_messages > 0 ? QString(" %1").arg(max_messages) : "") % CLRF;
...
inline void Client::setdebugmode(bool mode) {
    _debug_mode = mode;
}

inline bool Client::isOpen() {
    return m_socket.isValid();
}

inline uint64_t Client::request(const QString subject,
                                MessageCallback callback) {
    return request(subject, "", callback);
}

inline uint64_t Client::request(const QString subject, const QString message,
                                MessageCallback callback) {
